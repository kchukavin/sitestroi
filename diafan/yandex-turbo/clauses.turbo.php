...
$rows = DB::query_fetch_all("SELECT e.id, e.created, e.[name], e.[anons], e.[text], e.site_id FROM {clauses} AS e"
...
foreach ($rows as $row)
{
	...
	if (empty($last))
	{
		$last = date("D, d F Y H:i:s O", $row['created']);
	}
	$items .= "
	<item turbo=\"true\">
		...
		<pubDate>".date("D, d F Y H:i:s O", $row['created'])."</pubDate>"
