 ...
 * Похожие элементы
 */
class rel2_elements_admin extends Diafan
{
	/**
	...
	public function ajax()
	{
		...
		if ($_POST["action"] != "show_rel2_elements")
		{
		...
		if ( ! empty($_POST["action"]))
		{
			switch ($_POST["action"])
			{
				...
				case 'show_rel2_elements':
					$this->show_rel2_elements();
					break;
				case 'delete_rel2_element':
					$this->delete_rel2_element();
			}
		}
	}
	 ...
	 * @return void
	 */
	private function rel2_elements()
	{
		...
		if ($_POST["element_id"] != $_POST["rel2_id"] &&
			! DB::query_result("SELECT id FROM {".$this->diafan->table."_rel2} WHERE element_id=%d AND rel2_element_id=%d LIMIT 1", $_POST["element_id"], $_POST["rel2_id"])
			&& (empty($_POST["rel2_two_sided"]) || ! DB::query_result("SELECT id FROM {".$this->diafan->table."_rel2} WHERE rel2_element_id=%d AND element_id=%d LIMIT 1", $_POST["element_id"], $_POST["rel2_id"])))
		{
			DB::query("INSERT INTO {".$this->diafan->table."_rel2} (element_id, rel2_element_id) VALUES (%d, %d)", $_POST["element_id"], $_POST["rel2_id"]);
		}
		...
		$rows = DB::query_fetch_all("SELECT s.id, s.[".$name."], s.site_id FROM {".$this->diafan->table."} AS s"
				...
				.(! empty($_POST["rel2_two_sided"]) ? " OR s.id=r.element_id AND r.rel2_element_id=%d" : "")
		...
		foreach ($rows as $row)
		{
			...
			$this->result["data"] .= '
			<div class="rel2_element" element_id="'.$element_id.'" rel2_id="'.$row["id"].'">'
	 ...
	 * @return void
	 */
	private function show_rel2_elements()
	{
		if($this->diafan->variable_list("name", "variable"))
		{
			$name = $this->diafan->variable_list("name", "variable");
		...
		if (empty($_POST["page"]))
		{
			...
			if (! isset($_POST["search"]) && ! isset($_POST["cat_id"]))
			{
				...
				<form><div class="infofield">'.$this->diafan->_('Поиск').'</div> <input type="text" class="rel2_module_search">';
				if($this->diafan->configmodules("cat", $this->diafan->_admin->module))
				{
					...
					$list.= ' <select class="rel2_module_cat_id"><option value="">'.$this->diafan->_('Все').'</option>'.$this->diafan->get_options($cats, $cats[0], $vals).'</select>';
				}
				$list.= '</form><div class="rel2_all_elements_container">';
			}
		}
		else
		{
		...
		$list .= '<div class="rel2_all_elements">';
		$rel2_elements = array();
		if ($_POST["element_id"])
		{
			$rel2_elements = DB::query_fetch_value("SELECT rel2_element_id FROM {".$this->diafan->table."_rel2} WHERE element_id=%d", $_POST["element_id"], "rel2_element_id");
			if(! empty($_POST["rel2_two_sided"]))
			{
				$new_rel2_elements = DB::query_fetch_value("SELECT element_id FROM {".$this->diafan->table."_rel2} WHERE rel2_element_id=%d", $_POST["element_id"], "element_id");
				$rel2_elements = array_merge($rel2_elements, $new_rel2_elements);
			}
		}
		...
		foreach ($rows as $row)
		{
			...
			$list .= '<div class="rel2_module'.(in_array($row["id"], $rel2_elements) ? ' rel2_module_selected' : '').'" element_id="'.$row["id"].'">
			<div>
		...
		$list .= '</div><div class="clear rel2_module_navig paginator">';
	 ...
	 * @return void
	 */
	private function delete_rel2_element()
	{
		DB::query("DELETE FROM {".$this->diafan->table."_rel2} WHERE element_id=%d AND rel2_element_id=%d", $_POST['element_id'], $_POST['rel2_id']);
		if(! empty($_POST["rel2_two_sided"]))
		{
			DB::query("DELETE FROM {".$this->diafan->table."_rel2} WHERE rel2_element_id=%d AND element_id=%d", $_POST['element_id'], $_POST['rel2_id']);
		}
